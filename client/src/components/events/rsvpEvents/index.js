import React from "react";

function EventsListRSVP() {
  return (
    <div className="w-full">
      <h2 className="mt-6 text-center text-5xl font-bold tracking-tight text-gray-900">
        Your Events
      </h2>
      <header className="mt-6 text-left text-2xl  tracking-tight text-gray-900">Search through Events that you have created here!</header>
      
      
      {/* <header>Header</header>
      This page will have sample details of events listing and details about the
      page like title of the page, header, body, footer
      <footer>Footer</footer>
      <header>Header</header>
      This page will have sample details of events listing and details about the
      page like title of the page, header, body, footer
      <footer>Footer</footer>
      <header>Header</header>
      This page will have sample details of events listing and details about the
      page like title of the page, header, body, footer
      <footer>Footer</footer>
      <header>Header</header>
      This page will have sample details of events listing and details about the
      page like title of the page, header, body, footer
      <footer>Footer</footer>
      <header>Header</header>
      This page will have sample details of events listing and details about the
      page like title of the page, header, body, footer
      <footer>Footer</footer>
      <header>Header</header>
      This page will have sample details of events listing and details about the
      page like title of the page, header, body, footer
      <footer>Footer</footer>
      <header>Header</header>
      This page will have sample details of events listing and details about the
      page like title of the page, header, body, footer
      <footer>Footer</footer>
      <header>Header</header>
      This page will have sample details of events listing and details about the
      page like title of the page, header, body, footer
      <footer>Footer</footer>
      <header>Header</header>
      This page will have sample details of events listing and details about the
      page like title of the page, header, body, footer
      <footer>Footer</footer>
      <header>Header</header>
      This page will have sample details of events listing and details about the
      page like title of the page, header, body, footer
      <footer>Footer</footer>
      <header>Header</header>
      This page will have sample details of events listing and details about the
      page like title of the page, header, body, footer
      <footer>Footer</footer>
      <header>Header</header>
      This page will have sample details of events listing and details about the
      page like title of the page, header, body, footer
      <footer>Footer</footer>
      <header>Header</header>
      This page will have sample details of events listing and details about the
      page like title of the page, header, body, footer
      <footer>Footer</footer>
      <header>Header</header>
      This page will have sample details of events listing and details about the
      page like title of the page, header, body, footer
      <footer>Footer</footer>
      <header>Header</header>
      This page will have sample details of events listing and details about the
      page like title of the page, header, body, footer
      <footer>Footer</footer>
      <header>Header</header>
      This page will have sample details of events listing and details about the
      page like title of the page, header, body, footer
      <footer>Footer</footer>
      <header>Header</header>
      This page will have sample details of events listing and details about the
      page like title of the page, header, body, footer
      <footer>Footer</footer>
      <header>Header</header>
      This page will have sample details of events listing and details about the
      page like title of the page, header, body, footer
      <footer>Footer</footer>
      <header>Header</header>
      This page will have sample details of events listing and details about the
      page like title of the page, header, body, footer
      <footer>Footer</footer>
      <header>Header</header>
      This page will have sample details of events listing and details about the
      page like title of the page, header, body, footer
      <footer>Footer</footer>
      <header>Header</header>
      This page will have sample details of events listing and details about the
      page like title of the page, header, body, footer
      <footer>Footer</footer>
      <header>Header</header>
      This page will have sample details of events listing and details about the
      page like title of the page, header, body, footer
      <footer>Footer</footer>
      <header>Header</header>
      This page will have sample details of events listing and details about the
      page like title of the page, header, body, footer
      <footer>Footer</footer>
      <header>Header</header>
      This page will have sample details of events listing and details about the
      page like title of the page, header, body, footer
      <footer>Footer</footer>
      <header>Header</header>
      This page will have sample details of events listing and details about the
      page like title of the page, header, body, footer
      <footer>Footer</footer>
      <header>Header</header>
      This page will have sample details of events listing and details about the
      page like title of the page, header, body, footer
      <footer>Footer</footer>
      <header>Header</header>
      This page will have sample details of events listing and details about the
      page like title of the page, header, body, footer
      <footer>Footer</footer>
      <header>Header</header>
      This page will have sample details of events listing and details about the
      page like title of the page, header, body, footer
      <footer>Footer</footer>
      <header>Header</header>
      This page will have sample details of events listing and details about the
      page like title of the page, header, body, footer
      <footer>Footer</footer>
      <header>Header</header>
      This page will have sample details of events listing and details about the
      page like title of the page, header, body, footer
      <footer>Footer</footer>
      <header>Header</header>
      This page will have sample details of events listing and details about the
      page like title of the page, header, body, footer
      <footer>Footer</footer>
      <header>Header</header>
      This page will have sample details of events listing and details about the
      page like title of the page, header, body, footer
      <footer>Footer</footer>
      <header>Header</header>
      This page will have sample details of events listing and details about the
      page like title of the page, header, body, footer
      <footer>Footer</footer>
      <header>Header</header>
      This page will have sample details of events listing and details about the
      page like title of the page, header, body, footer
      <footer>Footer</footer>
      <header>Header</header>
      This page will have sample details of events listing and details about the
      page like title of the page, header, body, footer
      <footer>Footer</footer>
      <header>Header</header>
      This page will have sample details of events listing and details about the
      page like title of the page, header, body, footer
      <footer>Footer</footer>
      <header>Header</header>
      This page will have sample details of events listing and details about the
      page like title of the page, header, body, footer
      <footer>Footer</footer>
      <header>Header</header>
      This page will have sample details of events listing and details about the
      page like title of the page, header, body, footer
      <footer>Footer</footer>
      <header>Header</header>
      This page will have sample details of events listing and details about the
      page like title of the page, header, body, footer
      <footer>Footer</footer>
      <header>Header</header>
      This page will have sample details of events listing and details about the
      page like title of the page, header, body, footer
      <footer>Footer</footer>
      <header>Header</header>
      This page will have sample details of events listing and details about the
      page like title of the page, header, body, footer
      <footer>Footer</footer>
      <header>Header</header>
      This page will have sample details of events listing and details about the
      page like title of the page, header, body, footer
      <footer>Footer</footer>
      <header>Header</header>
      This page will have sample details of events listing and details about the
      page like title of the page, header, body, footer
      <footer>Footer</footer>
      <header>Header</header>
      This page will have sample details of events listing and details about the
      page like title of the page, header, body, footer
      <footer>Footer</footer> */}
    </div>
  );
}

export default EventsListRSVP;
