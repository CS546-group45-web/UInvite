const dbConnection = require('./config/mongoConnection');
const data = require('./data/');
const { addRating, add_guests } = require('./data/events');
const events = data.events;
const comments = data.comments;
const users = data.users;


const main = async () => {
    const db = await dbConnection.dbConnection();
    await db.dropDatabase();
    let khoaId = 0;
    try {
        const firstName = "   Khoa   ";
        const lastName = "  Tran    "
        const email = " ktran7@stevens.edu "
        const username = "  Khoatran  ";
        const password = "Qwertyuiop123";
        const phone = "  2017908762 ";
        const dob = "  11/12/2002  ";
        const gender = " male "
        const khoa = await users.createUser(firstName,lastName,email,username,password,phone,dob, gender);
        khoaId = khoa.toString();
    } catch (error) {
        console.error(error);
    }
    let mikeId = 0;
    try {
        const firstName = "  Mike   ";
        const lastName = " Nguyen    "
        const email = "   kulohugo123@gmail.com   "
        const username = "  mikenguyen ";
        const password = "Qwert11iop123";
        const phone = "  9737908762 ";
        const dob = "  08/01/1999  ";
        const gender = " male "
        const mike = await users.createUser(firstName,lastName,email,username,password,phone,dob, gender);
        mikeId = mike.toString();
    } catch (error) {
        console.error(error);
    }
    let tarunId = 0;
    try {
        const firstName = "   Tarun   ";
        const lastName = "   Dadlani   "
        const email = " tdadlani@stevens.edu "
        const username = "  Tarunmasupalli  ";
        const password = "Qwert123op123";
        const phone = "  2017902222 ";
        const dob = "  10/01/2000  ";
        const gender = " male "
        const tarun = await users.createUser(firstName,lastName,email,username,password,phone,dob, gender);
        tarunId = tarun.toString();
    } catch (error) {
        console.error(error);
    }
    let sumanId = 0;
    try {
        const firstName = "   Suman   ";
        const lastName = "  Mohanty   "
        const email = " smohanty@stevens.edu "
        const username = "  sumanmohanty  ";
        const password = "Qwerty111p123";
        const phone = "  2017903333 ";
        const dob = "  01/14/1999  ";
        const gender = " female "
        const suman = await users.createUser(firstName,lastName,email,username,password,phone,dob, gender);
        sumanId = suman.toString();
    } catch (error) {
        console.error(error);
    }
    let maniId = 0;
    try {
        const firstName = "   Mani   ";
        const lastName = "  Masupalli   "
        const email = " mmasupalli@stevens.edu "
        const username = "  manimasupalli  ";
        const password = "Qwertyba11p123";
        const phone = "  2017901111 ";
        const dob = "  05/30/1998  ";
        const gender = " male "
        const mani = await users.createUser(firstName,lastName,email,username,password,phone,dob, gender);
        maniId = mani.toString();
    } catch (error) {
        console.error(error);
    }
    let arjunId = 0;
    try {
        const firstName = "   Arjun   ";
        const lastName = "  Khatri   "
        const email = " akhatri@stevens.edu "
        const username = "  arjunkhatri  ";
        const password = "Qwert1122p123";
        const phone = "  2017904567 ";
        const dob = "  09/23/2001  ";
        const gender = " male "
        const arjun = await users.createUser(firstName,lastName,email,username,password,phone,dob, gender);
        arjunId = arjun.toString();
    } catch (error) {
        console.error(error);
    }
    let birthdayId = 0;
    try {
        const user_id = arjunId;
        const event_title = "   Birthday Party      ";
        const organizer_name = "    Arjun   ";
        const description = " This is a birthday party for my brother! Cake included, gifts not required but encouraged.   ";
        const start_date_time = "   2022-12-30T18:00:00.000Z   ";
        const end_date_time = "   2022-12-30T21:00:00.000Z  ";
        const address = "92 River St, Hoboken, NJ 07030";
        const type = "in-person"; 
        const tags = ["birthday", "food", "strong17", " gift not required"];
        const birthday = await events.createEvent(user_id,event_title,organizer_name,description,start_date_time,end_date_time,address,type,tags)
        birthdayId = birthday;
    } catch (error) {
        console.error(error);
    }

    let graduationId = 0;
    try {
        const user_id = mikeId;
        const event_title = "   Khoa graduation    ";
        const organizer_name = "  Mikey   ";
        const description = " This is a surprise party for Khoa to celebrate his graduation.   ";
        const start_date_time = "   2023-05-27T11:00:00.000Z   ";
        const end_date_time = "   2023-05-27T14:00:00.000Z ";
        const address = "123 Hoboken Ave, Hoboken, NJ 07030";
        const type = "in-person";
        const tags = ["graduation", "food", "next chapter", " gift not required"];
        const graduation = await events.createEvent(user_id,event_title,organizer_name,description,start_date_time,end_date_time,address,type,tags);
        graduationId = graduation;
    } catch (error) {
        console.error(error);
    }

    let careerId = 0;
    try {
        const user_id = tarunId;
        const event_title = "   Career Fair    ";
        const organizer_name = "  Mikey   ";
        const description = " This is a career fair, recruiters from prestigous organizations are going to be interviewing candidates.   ";
        const start_date_time = "   2023-07-27T11:00:00.000Z   ";
        const end_date_time = "   2023-07-27T14:00:00.000Z ";
        const address = "111 Hoboken Rd, Hoboken, NJ 07030";
        const type = "in-person";
        const tags = ["career", "food", "next chapter", "interview"];
        const career = await events.createEvent(user_id,event_title,organizer_name,description,start_date_time,end_date_time,address,type,tags);
        careerId = career;
    } catch (error) {
        console.error(error);
    }

    try {
        let event = await events.add_guest(birthdayId, "ktran7@stevens.edu");
        event = await events.add_guest(birthdayId, "kulohugo123@gmail.com");
    }catch(error){
        console.error(error);
    }

    try {
        let event = await events.add_guest(graduationId, "akhatri@stevens.edu");
        event = await events.add_guest(graduationId, "smohanty@stevens.edu");
        event = await events.add_guest(graduationId, "tdadlani@stevens.edu");
        event = await events.add_guest(graduationId, "mmasupalli@stevens.edu");
    }catch(error){
        console.error(error);
    }

    try{
        let user = await users.rsvp_event(khoaId, birthdayId);
        user = await users.rsvp_event(mikeId, birthdayId);
        user = await users.rsvp_event(arjunId,graduationId);
        user = await users.rsvp_event(sumanId, graduationId);
    }catch(error){
        console.error(error);
    }

    try{
        await add_guests(careerId, ["akhatri@stevens.edu","smohanty@stevens.edu","mmasupalli@stevens.edu","kulohugo123@gmail.com","ktran7@stevens.edu"])
    }catch(error){
        console.error(error);
    }

    try {
        const comment = "This was a great event!";
        await comments.createComment(birthdayId, khoaId, comment);
    } catch (error) {
        console.error(error);
    }

    try {
        const comment = "I wanted to see more food.";
        await comments.createComment(birthdayId, tarunId, comment);
    } catch (error) {
        console.error(error);
    }

    try {
        const comment = "It was boring";
        await comments.createComment(graduationId, arjunId, comment);
    } catch (error) {
        console.error(error);
    }

    try {
        const comment = "I loved the decoration.";
        await comments.createComment(birthdayId, arjunId, comment);
    } catch (error) {
        console.error(error);
    }

    try {
        const comment = "We love the people.";
        await comments.createComment(graduationId, sumanId, comment);
    } catch (error) {
        console.error(error);
    }

    try {
        const comment = "It was too crowded.";
        await comments.createComment(graduationId, maniId, comment);
    } catch (error) {
        console.error(error);
    }


    try {
        const comment = "I love white castle";
        await comments.createComment(birthdayId, tarunId, comment);
    } catch (error) {
        console.error(error);
    }

    try {
        const comment = "I would love to be invited again.";
        await comments.createComment(graduationId, khoaId, comment);
    } catch (error) {
        console.error(error);
    }

    try {
        const comment = "when is the next party ? ";
        await comments.createComment(birthdayId, mikeId, comment);
    } catch (error) {
        console.error(error);
    }

const users_id = [khoaId,arjunId,tarunId,mikeId,sumanId,maniId];
    try {
        const max = Math.floor(Math.random() * 20);
        for(let i = 0; i < max; i++){
            await events.addRating(birthdayId,users_id[Math.floor(Math.random()*users_id.length)], Math.round(Math.random()*5*10)/10)
        }
    }catch(error){
        console.error(error);
    }

    try {
        const max = Math.floor(Math.random() * 20);
        for(let i = 0; i < max; i++){
            await events.addRating(graduationId,users_id[Math.floor(Math.random()*users_id.length)], Math.round(Math.random()*5*10)/10)
        }
    }catch(error){
        console.error(error);
    }

    console.log("RESULTS--------------------------------------------------------------")
    try{
        const all_events = await events.getAllEvents();
        console.log(all_events);
    }catch(error){
        console.error(error);
    }

    // console.log("BIRTHDAY commentS--------------------------------------------------------------")
    // try{
    //     const all_comments = await comments.getAllComments(birthdayId);
    //     console.log(all_comments);
    // }catch(error){
    //     console.error(error);
    // }

    // console.log("GRADUATION commentS--------------------------------------------------------------")
    // try{
    //     const all_comments = await comments.getAllComments(graduationId);
    //     console.log(all_comments);
    // }catch(error){
    //     console.error(error);
    // }

    console.log("All USERS--------------------------------------------------------------")
    try{
        const all_users = await users.getAllUsers();
        console.log(all_users);
    }catch(error){
        console.error(error);
    }

    console.log('Done seeding database');

    await dbConnection.closeConnection();
}

main();